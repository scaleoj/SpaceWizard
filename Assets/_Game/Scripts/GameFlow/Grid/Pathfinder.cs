    ...
    public class Pathfinder
    {
        ...
        public List<TileAttribute> FindPath(GameObject start, GameObject end)
        {
            ...
            while (openList.Count > 0)
            {
                ...
                {
                    if (tiles.node.layer != 9<<LayerMask.GetMask("Walkable") || tiles.node.GetComponent<TileContainer>().OccupiedGameObject != null || closedList.Contains(tiles))
                    {
                        continue;
                    }
        ...
        public List<TileAttribute> FindPathNoMove(GameObject start, GameObject end)
        {
            //Implementierung A*
            var startX = 0;
            var startY = 0;
